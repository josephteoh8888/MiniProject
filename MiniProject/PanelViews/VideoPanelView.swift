...
//                    feed.vcDataList.insert(contentsOf: l, at: 0)
...
//                    let y = feed.vcDataList[w]
//                    let z = feed.vcDataList[x]
...
//                            feed.vcDataList[feed.vcDataList.count - 1] = "c" //"a"
                            let vData = VideoData()
                            ...
                            feed.dataPaginateStatus = ""
//                            feed.vcDataList.insert(contentsOf: l, at: feed.vcDataList.count - 1)
                        ...
                        feed.videoCV?.reloadData()
//                        feed.reloadAllData() //test
                        print("api success asyncPaginateFetchFeed after \(feed.dataPaginateStatus), \(feed.vcDataList)")
                    }
                    else {
//                        feed.bufferDataList.append(contentsOf: l)
                        //test 2 > new append method
                        for i in l {
                            let vData = VideoData()
                            vData.setDataType(data: i)
                            vData.setData(data: i)
                            vData.setTextString(data: i)
                            feed.bufferDataList.append(vData)
                        }
                        feed.dataPaginateStatus = "buffer"
                    }
                }
