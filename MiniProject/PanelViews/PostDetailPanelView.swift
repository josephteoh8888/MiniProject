...
protocol PostDetailPanelDelegate : AnyObject {
    func didClickPostDetailPanelVcvClickPost(id: String, dataType: String, scrollToComment: Bool, pointX: CGFloat, pointY: CGFloat) //try
    ...
    func didStartPostDetailPanGesture(ppv : PostDetailPanelView)
    func didEndPostDetailPanGesture(ppv : PostDetailPanelView)
    func didStartOpenPostDetailPanel()
    func didFinishOpenPostDetailPanel()
    func didStartClosePostDetailPanel(ppv : PostDetailPanelView)
    func didFinishClosePostDetailPanel(ppv : PostDetailPanelView)
}
...
//        aStickyHeader.layer.opacity = 0.3
        ...
        let aBtn = UIView()
//        aBtn.backgroundColor = .ddmBlackDark
...
//        aMoreCBtn.backgroundColor = .ddmBlackDark
        ...
        aMoreCBtn.layer.cornerRadius = 15 //10
        //test > for sharing
...
//            direction = "na"
            ...
            if(dist >= distLimit) {
                self.delegate?.didStartClosePostDetailPanel(ppv: self)
                ...
                //test
                self.delegate?.didEndPostDetailPanGesture(ppv: self)
            }
...
//            })
            ...
            UIView.animate(withDuration: 0.5, delay: 0, options: [.curveEaseInOut], animations: {
                ...
                self.delegate?.didFinishClosePostDetailPanel(ppv : self)
            })
        } else {
            //test > stop video before closing panel
            self.destroyCell()
...
//            self.delegate?.didClickPostDetailClosePanel()
    ...
    func open(offX: CGFloat, offY: CGFloat, delay: CGFloat, isAnimated: Bool) {
        ...
        if(isAnimated) {
            self.delegate?.didStartOpenPostDetailPanel()
            ...
            UIView.animate(withDuration: 0.2, delay: delay, options: [.curveEaseInOut], //default: 0.2
                ...
                self.delegate?.didFinishOpenPostDetailPanel()
...
//                        self.vcDataList.append(postData)
    ...
    func configureFooterUI(data: String) {
        ...
        else if(data == "ep") { //error loading post
            errorText.text = "Unable to load post. Try again"
            errorRefreshBtn.isHidden = false
        }
        else if(data == "na") {
...
//        bottomBox.isHidden = false
    ...
    func destroyCell() {
        ...
        for cell in a.visibleCells {
            ...
            else if let b = cell as? HPostListBViewCell {
                b.destroyCell()
            }
        }
    }
...
//                            cSize = CGSize(width: cWidth, height: cWidth + descHeight)
                        ...
                        let pTopMargin = 20.0
                        let pContentHeight = cSize.height //280
                        let pHeight = pTopMargin + pContentHeight //40.0 for bottom container for description
                        contentHeight += pHeight
                    }
                    else if(l == "video") {
                        let cellWidth = self.frame.width
...
//                            cSize = CGSize(width: cWidth, height: cWidth + descHeight)
    ...
    //                                    cSize = CGSize(width: cWidth, height: cWidth + descHeight)
                ...
                let npTBottomMargin = 10.0
                let npText = "Post does not exist."
                let npContentHeight = estimateHeight(text: npText, textWidth: collectionView.frame.width - indentSize - 20.0, fontSize: 13)
                let npHeight = npTTopMargin + npContentHeight + npTBottomMargin + npTopMargin
                contentHeight += npHeight
            }
            else if(d == "us") {
...
extension PostDetailPanelView: HListCellDelegate {
    func hListDidClickVcvComment(vc: UICollectionViewCell, id: String, dataType: String, pointX: CGFloat, pointY: CGFloat) {
        ...
        else if vc is HCommentListViewCell {
            pausePlayingMedia()
            openComment()
        }
    }
    ...
    func hListDidClickVcvClickUser(id: String){
        ...
        pausePlayingMedia()
        delegate?.didClickPostDetailPanelVcvClickUser(id: id)
    }
        ...
        if let a = postCV {
            for cell in a.visibleCells {
                ...
                if(cell == vc) {
                    ...
                    delegate?.didClickPostDetailPanelVcvClickPost(id: id, dataType: dataType, scrollToComment: false, pointX: pointX1, pointY: pointY1)
        ...
        if let a = postCV {
            for cell in a.visibleCells {
                ...
                if(cell == vc) {
                    ...
                    delegate?.didClickPostDetailPanelVcvClickPhoto(id: id, pointX: pointX1, pointY: pointY1, view: view, mode: mode)
        ...
        if let a = postCV {
            for cell in a.visibleCells {
                ...
                if(cell == vc) {
                    ...
                    delegate?.didClickPostDetailPanelVcvClickVideo(id: id, pointX: pointX1, pointY: pointY1, view: view, mode: mode)
...
//        bottomBox.isHidden = true
    ...
    func didCClickComment(id: String, dataType: String, pointX: CGFloat, pointY: CGFloat){
        delegate?.didClickPostDetailPanelVcvClickPost(id: id, dataType: dataType, scrollToComment: true, pointX: pointX, pointY: pointY)
    }
    func didCClickShare(id: String, dataType: String){
        openShareSheet(oType: dataType, oId: id)
    }
    func didCClickPost(id: String, dataType: String, pointX: CGFloat, pointY: CGFloat){
        delegate?.didClickPostDetailPanelVcvClickPost(id: id, dataType: dataType, scrollToComment: false, pointX: pointX, pointY: pointY)
    }
...
extension PostDetailPanelView: UITextViewDelegate {
...
extension ViewController: PostDetailPanelDelegate{
    func didClickPostDetailPanelVcvClickPost(id: String, dataType: String, scrollToComment: Bool, pointX: CGFloat, pointY: CGFloat) {
