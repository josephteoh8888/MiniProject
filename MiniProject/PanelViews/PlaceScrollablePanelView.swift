//
//  PlaceScrollablePanelView.swift
//  MiniProject
//
...
protocol PlaceScrollablePanelDelegate : AnyObject {
    ...
    func didPClickSoundPlaceScrollable(id: String, pointX: CGFloat, pointY: CGFloat, view: UIView, mode: String)
...
class PlaceScrollablePanelView: ScrollablePanelView{
    ...
    weak var delegate : PlaceScrollablePanelDelegate?
...
//    var halfModeMapPadding : CGFloat = 200.0 //260, default: 210 in iphone 6s, 310 in iphone 11
    ...
    func initialize() {
        ...
        UIView.animate(withDuration: 0.2, animations: {
            ...
            self.changePlacePanelMode(panelMode: self.PANEL_MODE_HALF) //test
...
//        aNameText.text = "Petronas Twin Tower"
        aNameText.text = ""
...
//        aUsernameAText.text = "@michael" //Owner | @michael
        aUsernameAText.text = ""
...
//                cell.delegate = self
    ...
    @objc func onPanelPanGesture(gesture: UIPanGestureRecognizer) {
        ...
        if(gesture.state == .began) {
            ...
            if(self.currentPanelMode != self.PANEL_MODE_EMPTY) {
                self.delegate?.didFinishMapChangePlaceScrollable()
            }
        }
    }
    ...
    override func close(isAnimated: Bool) {
        ...
        //test
        delegate?.didStartMapChangePlaceScrollable()
        self.changePlacePanelMode(panelMode: self.PANEL_MODE_EMPTY) //test
...
//                self.pauseFeedPlayingMedia()
                ...
                self.delegate?.didClickClosePlaceScrollablePanel()
...
//            self.pauseFeedPlayingMedia()
    ...
    func changePlacePanelMode(panelMode : String) {
        print("change correctMapPadding: \(panelMode)")
        if(panelMode == PANEL_MODE_EMPTY) {
            currentMapPaddingBottom = 0
            delegate?.didChangeMapPaddingPlaceScrollable(y: 0)
        } else {
            currentMapPaddingBottom = halfModeMapPadding
            delegate?.didChangeMapPaddingPlaceScrollable(y: halfModeMapPadding)
        }
...
//        delegate?.didPClickSoundPlaceScrollable(id: "")
    ...
    @objc func onCloseClicked(gesture: UITapGestureRecognizer) {
        ...
        UIView.animate(withDuration: 0.2, animations: {
            ...
            self.panelTopCons?.constant = -self.halfModePanelHeight
            self.changePlacePanelMode(panelMode: self.PANEL_MODE_HALF)
        ...
        if(panelTopCons!.constant < -halfModePanelHeight) {
            ...
            if(self.currentPanelMode == self.PANEL_MODE_HALF) {
                if(currentMapPaddingBottom - y < 0) {
                    ...
                    delegate?.didChangeMapPaddingPlaceScrollable(y: currentMapPaddingBottom - y)
                ...
                if(currentMapPaddingBottom - gapY < 0) {
                    ...
                    delegate?.didChangeMapPaddingPlaceScrollable(y: currentMapPaddingBottom - gapY)
...
//                idx = self.getIntersectedIdx()
            ...
            self.scrollABTransition(scrollLevel: self.getScrollLevel())
            self.changePlacePanelMode(panelMode: panelMode) //test
            self.superview?.layoutIfNeeded()
...
//            b.reactToIntersectedVideo(intersectedIdx: idx)
    ...
    override func resumeActiveState() {
        print("placescrollableview resume active")
...
extension PlaceScrollablePanelView: UIScrollViewDelegate {
...
extension PlaceScrollablePanelView: HighlightCellDelegate {
...
extension PlaceScrollablePanelView: CommentScrollableDelegate{
    ...
    func didCClickSound(id: String, pointX: CGFloat, pointY: CGFloat, view: UIView, mode: String){
        delegate?.didPClickSoundPlaceScrollable(id: id, pointX: pointX, pointY: pointY, view: view, mode: mode)
    }
