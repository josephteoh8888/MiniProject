...
//        aBtn.backgroundColor = .ddmDarkColor
    ...
    //        aBtn.topAnchor.constraint(equalTo: userPanel.topAnchor, constant: 30).isActive = true
        aBtn.topAnchor.constraint(equalTo: panelView.topAnchor, constant: 20).isActive = true //50
...
//        panelTitleText.font = .systemFont(ofSize: 14) //default 14
        ...
        panelTitleText.text = "Select Photo"
...
//        panelTitleText.addGestureRecognizer(UITapGestureRecognizer(target: self, action: #selector(onCommentTitleClicked)))
        ...
        let panelTitleBtn = UIImageView(image: UIImage(named:"icon_round_arrow_down")?.withRenderingMode(.alwaysTemplate))
//            aArrowBtn.image = UIImage(named:"icon_round_arrow_down")?.withRenderingMode(.alwaysTemplate)
        panelTitleBtn.tintColor = .white
        panelView.addSubview(panelTitleBtn)
        panelTitleBtn.translatesAutoresizingMaskIntoConstraints = false
        panelTitleBtn.leadingAnchor.constraint(equalTo: panelTitleText.trailingAnchor).isActive = true
        panelTitleBtn.centerYAnchor.constraint(equalTo: panelTitleText.centerYAnchor).isActive = true
        panelTitleBtn.heightAnchor.constraint(equalToConstant: 26).isActive = true //ori 26
        panelTitleBtn.widthAnchor.constraint(equalToConstant: 26).isActive = true
        panelTitleBtn.isHidden = true
        ...
        if(isStoragePermissionNotDetermined) {
            ...
            PHPhotoLibrary.requestAuthorization(for: .readWrite) { [weak self] status in
                ...
                if status == .authorized {
                    ...
                    s.aLoggedOutBox.isHidden = true
                    s.isStoragePermissionNotDetermined = false
                    s.retrievePhotoFilesFromCameraRoll()
                    
//                    if(s.isMultiSelect) {
//                        s.multiToolPanel.isHidden = false
//                        s.coverBottomCon.isHidden = false
//                    }
                }
            }
        } else {
...
//            self?.vDataList.append(object)
    ...
    func refreshGridSelectedOrder() {
        ...
        for i in selectedGridAssetDataList {
            ...
            guard let b = vc as? GridPhotoRollViewCell else {
                return
            }
...
//        print("userpanel collection: \(section)")
        return UIEdgeInsets(top: 0.0, left: 20.0, bottom: 100.0, right: 20.0) //bottom 50
    }
...
extension CameraPhotoRollPanelView: GridAssetDelegate {
        ...
        if let a = vCV {
            for cell in a.visibleCells {
                ...
                if(cell == vc) {
                    ...
                    if(isMultiSelect) {
                        if let gridAsset = vc as? GridPhotoRollViewCell {
...
//                                        if(!self.multiSelectDataList.contains(url)) {
                                        ...
                                        print("Asset URL[]: \(self.selectedGridAssetDataList)")
                        ...
                        model.requestContentEditingInput(with: options) { (input, _) in
                            if let url = input?.fullSizeImageURL {
                                DispatchQueue.main.async {
                                    ...
                                    self.delegate?.didClickMultiPhotoSelect(urls: self.multiSelectDataList)
...
extension CameraPhotoRollPanelView: MultiSelectedCellDelegate {
